...
def main():
    ...
    '''
    The way python GIL works, compute_some needs to be in separate process
    i.e multiprocessing needs to used
    '''
    compute_some()
    compute_some()
    compute_some()
    '''
    this can be done with asyncio and await & aiohttp
    cpython releases GIL when you talk to network, 
    thus multithreading is also helpful
    '''
    download_some()
    download_some()
    download_some_more()
